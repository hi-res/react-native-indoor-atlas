const ReactNative = require('react-native'),
      ...
      NativeEventEmitter = ReactNative.DeviceEventEmitter || ReactNative.NativeAppEventEmitter,
...
const eventEmitterFn = function(method) {
  return function() {
    const retVal = this.events[method].apply(
      this.events,
      [].slice.apply(arguments)
    )

    if (retVal === this.events) {
      return this
    }
    return retVal
  }
...
function bindNativeListeners(events) {
  Object.keys(IALocationManagerEvents)
    ...
    .map(ev => {
      ...
      NativeEventEmitter.addListener(`RNIA.${ev}`, handler)
    })
}
